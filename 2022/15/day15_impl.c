...
void calculate(char** data, uint64_t data_size, int64_t* part1, int64_t* part2, int64_t param1, int64_t param2) {
    ...
    for (uint64_t i=0; i<list_tuple3_i64_size(sensor_distances); ++i) {
        ...
        if (i64abs(y-param1) <= dist) {
            rangeset_set(rs, x - (dist - i64abs(y-param1)), x + (dist - i64abs(y-param1)));
        }
    }
    
    *part1 = rangeset_popcnt(rs);
    
    rangeset_free(rs);
    
    // Above we have counted "beacon" squares as impossible to contain beacons, so now we need to apply
